 ...
 * @brief       channel estimation using LS copy from NR
 * @author      shenyihao
 * @date        2025.3.12
 ...
 */
 ...
 typedef short __v2048i16 __attribute__((ext_vector_type(8448)));
 typedef char  __v4096i8 __attribute__((ext_vector_type(8448)));
 
 #define LEFT_SHIFT  0
 #define RIGHT_SHIFT 1
 VENUS_INLINE __v2048i16 cyclic_shift_7168_16(__v2048i16 tgtvec, int shift_length, int shift_direction,
                                              int vectorLength) {
   __v2048i16 result;
 ...
 VENUS_INLINE __v4096i8 cyclic_shift_14336_8(__v4096i8 tgtvec, int shift_length, int shift_direction,
                                              int vectorLength) {
   __v4096i8 result;
   __v2048i16 tmp_index;
   vclaim(tmp_index);
 ...
 int Task_lteChannelEstimate(__v4096i8 rxData_real_in, __v4096i8 rxData_imag_in, __v4096i8 dmrs_real, __v4096i8 dmrs_imag,
  ...
  //  printf("%d\n",&input_subcarrierLength.data);
   ...
   __v4096i8 rxData_real;
   __v4096i8 rxData_imag;
   rxData_real = vsadd(rxData_real_in, 0, MASKREAD_OFF, 1024 * data_symbol_length);
   rxData_imag = vsadd(rxData_imag_in, 0, MASKREAD_OFF, 1024 * data_symbol_length);
   ...
   vbrdcst(const_value_8bit, dmrs_interval, MASKREAD_OFF, 14336);
   __v4096i8 global_coef_h;
   vclaim(global_coef_h);
   global_coef_h = vrem(const_value_8bit, global_coef_h_in, MASKREAD_OFF, 14336);
   ...
   for (int i = 0; i < dmrs_symbol_length; ++i) {
     ...
     temp_h_shift_real = cyclic_shift_14336_8(temp_h_real, 1, LEFT_SHIFT, dmrs_ref_length);
     temp_h_shift_imag = cyclic_shift_14336_8(temp_h_imag, 1, LEFT_SHIFT, dmrs_ref_length);
     ...
     VSPM_CLOSE();
     base_dmrs_shuffle_index = cyclic_shift_7168_16(base_dmrs_shuffle_index, left_index, RIGHT_SHIFT, subcarrierLength);
     weight_dmrs_shuffle_index =
         cyclic_shift_7168_16(weight_dmrs_shuffle_index, left_index, RIGHT_SHIFT, subcarrierLength);
     coef_h = cyclic_shift_14336_8(coef_h, left_index, RIGHT_SHIFT, subcarrierLength);
     vbarrier();
     VSPM_OPEN();
   ...
   vreturn(h_real, 8448, h_imag, 8448);
 }