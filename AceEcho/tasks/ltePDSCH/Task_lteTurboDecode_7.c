...
int Task_lteTurboDecode_7(
    __v4096i8 softin1_x_B,__v4096i8 softin1_y_B,__v4096i8 e_p_B,
    ...
    vshuffle(zerostemp1,temp10,softin1_x_B,SHUFFLE_GATHER,B);
    vbrdcst(softin1_x_B,0,MASKREAD_OFF,fac);
    vshuffle(softin1_x_B,temp10,zerostemp1,SHUFFLE_SCATTER,B);
    softin1_x_B = vsadd(softin1_x_B,0,MASKREAD_OFF,fac);

    vbrdcst(zerostemp1,0,MASKREAD_OFF,fac);
    vshuffle(zerostemp1,temp10,softin1_y_B,SHUFFLE_GATHER,B);
    vbrdcst(softin1_y_B,0,MASKREAD_OFF,fac);
    vshuffle(softin1_y_B,temp10,zerostemp1,SHUFFLE_SCATTER,B);
    softin1_y_B = vsadd(softin1_y_B,0,MASKREAD_OFF,fac);
    ...
    */
    ...
    // for(short k=0; k<0;k++){
        ...
        if(k>0){
            ...
            */
            ...
            // a_add_index1 = vsadd(a_add_index1, 0,MASKREAD_OFF,length);  //136
            // a_add_index2 = vsadd(a_add_index2, 0,MASKREAD_OFF,length);
            // a_add_index3 = vsadd(a_add_index3, 0,MASKREAD_OFF,length);
            ...
            vshuffle(d4_B_ksub1,a_add_index4,d2_B,SHUFFLE_GATHER,length);
            ...
            // d1_B_ksub1 = vsadd(d1_B_ksub1, 0,MASKREAD_OFF,length);      //280
            // d2_B_ksub1 = vsadd(d2_B_ksub1, 0,MASKREAD_OFF,length);      //
            // d3_B_ksub1 = vsadd(d3_B_ksub1, 0,MASKREAD_OFF,length);      //
            // d4_B_ksub1 = vsadd(d4_B_ksub1, 0,MASKREAD_OFF,length);      //
            ...
            atemp2_8 = vsadd(atemp2_8,d4_B_ksub1,MASKREAD_OFF,length);      //
            //index++
            vsle(ad_vseqindex,bd_vseqindex,MASKREAD_OFF,MASKWRITE_ON,length);
            // a_add_index1 = vsadd(a_add_index1, 1,MASKREAD_ON,length);       //157
            // a_add_index2 = vsadd(a_add_index2, 1,MASKREAD_ON,length);
            // a_add_index3 = vsadd(a_add_index3, 1,MASKREAD_ON,length);
            // a_add_index4 = vsadd(a_add_index4, 1,MASKREAD_ON,length);       //160
            a_add_index1 = vsadd(a_add_index1,ones_8,MASKREAD_ON,length);       //157
            a_add_index2 = vsadd(a_add_index2,ones_8,MASKREAD_ON,length);
            a_add_index3 = vsadd(a_add_index3,ones_8,MASKREAD_ON,length);
            a_add_index4 = vsadd(a_add_index4,ones_8,MASKREAD_ON,length);       //160
            //index--
            vsle(bd_vseqindex,ad_vseqindex,MASKREAD_OFF,MASKWRITE_ON,length);
            ...
            // a_add_index1 = vsub(a_add_index1,ones_8,MASKREAD_ON,length);
            ...
            a_add_index1 = vsadd(a_add_index1, 0,MASKREAD_OFF,length);  //169
            a_add_index2 = vsadd(a_add_index2, 0,MASKREAD_OFF,length);
            a_add_index3 = vsadd(a_add_index3, 0,MASKREAD_OFF,length);
            a_add_index4 = vsadd(a_add_index4, 0,MASKREAD_OFF,length);
            atemp1_8 = vsadd(atemp1_8, 0,MASKREAD_OFF,length);     //160
            atemp2_8 = vsadd(atemp2_8, 0,MASKREAD_OFF,length);
            ...
            atemp1_8 = vsadd(atemp1_8,atemp2_8, MASKREAD_ON, length);   //得到a[k]
            atemp = vsadd(atemp1_8,0, MASKREAD_OFF, length); 
            ...
            n = vredmin8(m,MASKREAD_OFF,length);
            vbarrier();
            VSPM_OPEN();
            if(*(volatile unsigned char *) (vaddr(n)) ==0){     //若m中的最小值为0，表明atemp1_8存在大于64的值
                //判定前向是否溢出
                vrange(SW_tempindex,half_length);
            ...
            VSPM_CLOSE();
            atemp = vsadd(atemp1_8, 0,MASKREAD_OFF,length); //169
            // atemp1_index_8 = vsadd(atemp1_index_8, 0,MASKREAD_OFF,length);
            // atemp2_index_8 = vsadd(atemp2_index_8, 0,MASKREAD_OFF,length);
    ...
    // char words[30] = "SISO1 a b computing finished";
    // printf("-------%s---------\n",&words);
    ...
    vreturn(atemp1_8B,sizeof(atemp1_8B),softin1_x_B_out,sizeof(softin1_x_B_out), d1_B,sizeof(d1_B),d2_B,sizeof(d2_B) , a_p_B,sizeof(a_p_B), e_p_B,sizeof(e_p_B));
