...
int Task_lteTurboDecode_2(__v4096i8 atemp1_8B,short_struct trblklength,short_struct SW_value){
    ...
    // if(flag==1){           //只有一段
    //拆第一段
    atemp1_8B = vsadd(atemp1_8B,0,MASKREAD_OFF,4096);
    split_index1 = vsadd(split_index1,0,MASKREAD_OFF,2048);
    vshuffle(SW_temp,split_index1,atemp1_8B,SHUFFLE_GATHER,length2);   //拆出atemp1_8B中的所有a1放入a1中对应的位置
    SW_temp = vsadd(SW_temp,0,MASKREAD_OFF,4096);

    SW_temp = vsadd(SW_temp,0,MASKREAD_OFF,4096);
    split_index2 = vsadd(split_index2,0,MASKREAD_OFF,2048);
    vshuffle(a1_B,split_index2,SW_temp,SHUFFLE_SCATTER,length2);
    a1_B = vsadd(a1_B,0,MASKREAD_OFF,4096);
...
// }else if(flag==3){     //分三段
    ...
    // char words[30] = "SISO1 data split finished";
    // printf("-------%s---------\n",&words);
