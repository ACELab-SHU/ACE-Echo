...
int Task_lteEqualize(__v4096i8 in_real, __v4096i8 in_imag, __v4096i8 hest_real0, __v4096i8 hest_imag0, __v4096i8 hest_real1, __v4096i8 hest_imag1, __v4096i8 hest_real2, __v4096i8 hest_imag2, __v4096i8 hest_real3, __v4096i8 hest_imag3,
  __v2048i16 index0145, __v2048i16 index048, short_struct input_cellrefp, short_struct input_sequence_length, short_struct NDLRB, short_struct input_data_symbol_length) {
  
  short Nt  = input_cellrefp.data;
  short inLength = input_sequence_length.data;
  short subcarrierLength   = NDLRB.data * 12;
...
/*----------------------------out and csi----------------------------------*/  
 ...
 /*----------------------------t2----------------------------------*/
  ...
  csi_tmp1 = vmul(hest_real0, hest_real0, MASKREAD_OFF, inLength);
  csi_tmp2 = vmul(hest_imag0, hest_imag0, MASKREAD_OFF, inLength);
  vsetshamt(0);

  csi = vsadd(csi_tmp1, csi_tmp2, MASKREAD_OFF, inLength);
  csi = vsadd(csi, nVar, MASKREAD_OFF, inLength);
  ...
  vsetshamt(fractionLength);
  hest_real0 = vdiv(csi, hest_real0, MASKREAD_OFF, inLength);
  hest_imag0 = vdiv(csi, hest_imag0, MASKREAD_OFF, inLength);
  vsetshamt(0);

  vsetshamt(fractionLength);
  pbchEq_tmp1 = vmul(hest_real0, in_real, MASKREAD_OFF, inLength);
  pbchEq_tmp2 = vmul(hest_imag0, in_imag, MASKREAD_OFF, inLength);
  vsetshamt(0);
  out_real = vsadd(pbchEq_tmp1, pbchEq_tmp2, MASKREAD_OFF, inLength);

  vsetshamt(fractionLength);
  pbchEq_tmp3 = vmul(hest_real0, in_imag, MASKREAD_OFF, inLength);
  pbchEq_tmp4 = vmul(hest_imag0, in_real, MASKREAD_OFF, inLength);
  vsetshamt(0);
  out_imag = vssub(pbchEq_tmp4, pbchEq_tmp3, MASKREAD_OFF, inLength);
